...
void get_header_request_all_line(struct SessionRunParams * session_params_ptr)
{
    ...
    all_header_line = session_params_ptr->conninfo->hd_request.all_header_line;
...
void get_header_request_host(struct SessionRunParams * session_params_ptr)
{
    ...
    if (index == 100)
    {
        ...
        get_header_value_by_key(all_header_line[index], "Host", session_params_ptr->conninfo->hd_request.host);
...
void get_header_request_ua(struct SessionRunParams * session_params_ptr)
{
    ...
    if (index == 100)
        memcpy(session_params_ptr->conninfo->hd_request.user_agent, HEADER_VALUE_NOT_AVAIABLE, strlen(HEADER_VALUE_NOT_AVAIABLE) + 1);
    else
        get_header_value_by_key(all_header_line[index], "User-Agent", session_params_ptr->conninfo->hd_request.user_agent);

    printf("session_params_ptr->conninfo->hd_request.user_agent:%s\n", session_params_ptr->conninfo->hd_request.user_agent);
}
